...
/*
 * Weights follow tabletkeymaps/de.h, sDeQwertz. Row sums: 12 / 12 / 12 / 11.8 / 11.
 * The symbol key label is "+ ~ [  ]" for this layout, not "+ = [  ]": the tilde
 * sits on w here.
 */
...
KeyPad {
    ...
    Column {
        ...
        // DE_QWERTZ_NUMBERS_10(1) + KEY_1(2, cKey_Trackball)  [sum 12]
        KeyRow {
        ...
        // DE_QWERTZ_TOP_11(1) + KEY_1(1, Qt::Key_Backspace)  [sum 12]
        KeyRow {
            height: keyHeight
        ...
        // KEY_2(-0.2, A, Less) + DE_QWERTZ_MID_11(1) + KEY_1(0.8, Return)  [sum 12]
        KeyRow {
            height: keyHeight
            ...
            ReturnKey { weight: 0.8; label: "Enter"; shifted: "Enter" }
        }

        // KEY_1(0.8, Shift) + DE_QWERTZ_LOW_9(1) + KEY_1(2, Shift)  [sum 11.8]
        KeyRow {
            height: keyHeight
        ...
        // DE_QWERTZ_BOTTOM_ROW_DEFAULT. The symbol key is 2 units and only gives up
        // its right half when updateLanguageKey() needs to show a language key.
        Component {
            ...
            KeyRow {
                ...
                SymbolShiftKey { label: "+~[]"; shifted: "+~[]"; weight: languageMenuButton.visible ? 1 : 2 }
                LanguageKey    { id: languageMenuButton }
                SpaceKey       { weight: 5 }
                AnnotatedKey { label: "ß"; shifted: "?"; extended: ["ß", "?", "¿"]; extendedShifted: ["ß", "?", "¿"] }
                AnnotatedKey { label: "-"; shifted: "'"; extended: ["-", "'", "±", "¬", "`", "‚", "‘","’"]; extendedShifted: ["-", "'", "±", "¬", "`", "‚", "‘","’"] }
                DismissKey     { }
            }
        }

        // DE_QWERTZ_BOTTOM_ROW_EMAIL: space drops to SPACE_SIZE - 2.
        Component {
            id: contentTypeEmail
            KeyRow {
                height: keyHeight
                ...
                TabKey         { label: "Tab"; shifted: "Tab" }
                SymbolShiftKey { label: "+~[]"; shifted: "+~[]"; weight: languageMenuButtonEmail.visible ? 1 : 2 }
                LanguageKey    { id: languageMenuButtonEmail }
                UrlKey         { label: "@"; shifted: "@" }
                SpaceKey       { weight: 3 }
                UrlKey { label: ".com"; shifted: ".com"; extended: [".com", ".de", ".at", ".ch", ".net", ".org", ".edu", ".gov"] }
                AnnotatedKey { label: "ß"; shifted: "?"; extended: ["ß", "?", "¿"]; extendedShifted: ["ß", "?", "¿"] }
                AnnotatedKey { label: "-"; shifted: "'"; extended: ["-", "'", "±", "¬", "`", "‚", "‘","’"]; extendedShifted: ["-", "'", "±", "¬", "`", "‚", "‘","’"] }
                DismissKey     { }
            }
        ...
        Component {
            ...
            KeyRow {
                ...
                TabKey         { label: "Tab"; shifted: "Tab" }
                SymbolShiftKey { label: "+~[]"; shifted: "+~[]"; weight: languageMenuButtonUrl.visible ? 1 : 2 }
