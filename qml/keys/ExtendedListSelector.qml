...
Item {
    ...
    property int keyHeight: Units.gu(6.3) //currentlyAssignedKey ? currentlyAssignedKey.height : 0
    property int keyWidth: Units.gu(8.0) //currentlyAssignedKey ? currentlyAssignedKey.width : 0
    ...
    Item {
        ...
        width: Math.max(Units.gu(8.0) * (keyRepeater.count <=4 ? keyRepeater.count : 4), rowOfKeys.width + 10*2)
        height: ((Units.gu(3.0) + numberOfLines * Units.gu(6.3)))
    ...
    Flow {
        ...
        width: Math.min(keyRepeater.count, 4) * Units.gu(8.0)
